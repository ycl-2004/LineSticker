...
Prompt 的固定文案集中在 [`app.js`](app.js) 的 `buildAIPrompt()`；這個函式會在按下「一鍵複製專業生圖提示詞」時組合內容。可直接改函式內 `return [ ... ]` 的文字行：

- 想換人物規則、動作限制或文字排版：改「角色一致性」或「文字與情境」段落。
- 想換畫風、背景、描邊、色彩：改「視覺風格」段落。
- 想新增不能出現的東西：改「嚴格禁止」段落。

不要把 `sourceSpec.imageWidth`、`sourceSpec.imageHeight`、`config.columns`、`config.rows` 或 `state.selectedCount` 改成固定數字；它們會依目前選擇的張數與生成品質自動輸出正確的尺寸和網格。使用者每格填入的文字則由 `getCurrentPhrases()` 取得，會自動插入 Prompt 的「文字與情境」段落。
...
- 選擇 8 張時，工具預期來源是 2 欄 × 4 行（外框比例 1:2）。若上傳 4 欄 × 2 行（2:1）大圖，比例檢查會正確阻止裁切，避免切出錯誤順序。
